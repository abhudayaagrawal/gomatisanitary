/**
 * GMT BROCHURE — Sheet-to-Website data API.
 *
 ...
 *
 * doGet() returns the live product list as JSON. In-cell product images are
 * expensive to re-host (must download + re-upload each one), so that work is
 * cached: each call only processes images that are new or changed since the
 * last sync (capped per call so a single click never times out), and a
 * "Publish All Images Now" sheet menu item is provided to force a full catch-up
 * pass after a big bulk import of new photos.
 */
